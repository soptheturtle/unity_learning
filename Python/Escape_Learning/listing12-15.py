...
player_image_shadow = PLAYER_SHADOW["down"][0] #2
...
objects = {
	...
	21: [images.door, images.door_shadow, "The airlock door. For safety reasons, it requires two person operaions."],
	...
	27: [images.map, images.full_shadow, "The screen says the crash site was Sector: "\
		+ str(LANDER_SECTOR) + " // X: " + str(LANDER_X) + "// Y: " + str(LANDER_Y)],
	...
	33: [images.robot_arm, images.robot_arm_shadow, "A robot arm, used for heavy liftin"],
...
scenery = {
    ...
    35: [[16,2,9], [16,2,2], [16,3,3], [16,3,8], [16,8,9], [16,8,2], [16,1,8],
         [16,1,3], [12,8,6], [12,9,4], [12,9,8], [15,4,6], [12,7,1], [12,7,11]],
    36: [[4,3,1], [9,1,7], [8,1,8], [8,1,9], [5,5,4], [6,5,7], [10,1,1], [12,1,2]],
    37: [[48,3,1], [48,3,2], [48,7,1], [48,5,2], [48,5,3], [48,7,2], [48,9,2],
         [48,9,3], [48,11,1], [48,11,2]],
...
for room_coordinate in range(0,13):
	for room_number in [1, 2, 3, 4, 5]: # Add top fency
...
#using room data, scenery data and prop data.
	...
	global top_left_x, top_left_y, wall_transprency_frame
	room_data = GAME_MAP[current_room] #2
	room_name = room_data[0]
	room_height = room_data[1]
	room_width = room_data[2]

	floor_type = get_floor_type() #3
	...
	if room_data[3]: #If exit at top of this room #10
	...
	center_y = int(HEIGHT/2) #Center of game window #1
	center_x = int(WIDTH/2) #2
	room_pixel_width = room_width * TILE_SIZE #Size of room in pixels #3
	room_pixel_height = room_height * TILE_SIZE #4
	top_left_x = center_x - 0.5 * room_pixel_width
	top_left_y = (center_y - 0.5 * room_pixel_height) + 110

	for prop_number, prop_info in props.items(): #1
		prop_room = prop_info[0] #2
		prop_y = prop_info[1]
		prop_x =  prop_info[2]
		if (prop_room == current_room and room_map[prop_y][prop_x] in [0,39,2]): #3,4
			room_map[prop_y][prop_x] = prop_number #5
			image_here = objects[prop_number][0] #6
			image_width = image_here.get_width()
			image_width_in_tiles = int(image_width / TILE_SIZE)
			for tile_number in range(1, image_width_in_tiles): #7
...
def game_loop(): #1
	global player_x, player_y, current_room #2
	...
	#Save player's current position 5
	old_player_x = player_x
	old_player_y = player_y

	#Move if key is pressed 6
	if player_frame == 0:
		if keyboard.right:
	...
	if keyboard.tab and len(in_my_pockets) > 0: #1
		selected_item += 1 #2
		if selected_item > len(in_my_pockets) - 1: #3
			selected_item = 0 
		item_carrying = in_my_pockets[selected_item] #4
		display_inventory() #5
		time.sleep(0.25)

	if keyboard.d and item_carrying: #6
		drop_object(old_player_y, old_player_x) #7
	...
	if keyboard.x:  #1
		current_room = int(input("Enter room number: ")) #2
		player_x = 2 #3
		player_y = 2
		generate_map() #4
		start_room() #5
	...
	#If the player is standing somewhere they shouldn't, move them back. 7
...
def draw_image(image, y, x): #1
	screen.blit(image,   #2
				(top_left_x + (x*TILE_SIZE),
				top_left_y + (y * TILE_SIZE) - image.get_height()))

def draw_shadow(image, y, x): #3
	screen.blit(image,
				(top_left_x + (x*TILE_SIZE), 
				top_left_y + (y*TILE_SIZE)))

def draw_player():
	player_image = PLAYER[player_direction][player_frame] #4
	draw_image(player_image, player_y + player_offset_y, #5
			   player_x + player_offset_x)
	player_image_shadow = PLAYER_SHADOW[player_direction][player_frame] #6
	draw_shadow(player_image_shadow, player_y + player_offset_y, #7
		        player_x + player_offset_x)

def draw():
	if game_over:
		return
	# Clear the game arena area. #1
	box = Rect((0, 150), (800, 600))
	screen.draw.filled_rect(box, RED)
	box = Rect ((0, 0), (800, top_left_y + (room_height - 1)* 30))
	screen.surface.set_clip(box) #2
	floor_type = get_floor_type()

	for y in range(room_height): #Lay down floor tiles, then items on floor. #3
	...
	# Pressure pad in room 26 is added here, so props can go on top of it.  #4
	if current_room == 26:
	...
	for y in range(room_height): #5
		for x in range(room_width):
			...
			if item_here not in items_player_may_stand_on + [255]:
				...
				if (current_room in outdoor_rooms #6
					and y == room_height - 1
					...
					image = PILLARS[wall_transprency_frame]

				draw_image(image, y, x)

				if objects[item_here][1] is not None: #if object has a shadow #7
					shadow_image = objects[item_here][1]
					# if shadow might need horizontal tiling
					if shadow_image in [images.half_shadow, images.full_shadow]: #8
						shadow_width = int(image.get_width() / TILE_SIZE)
						# Use shadow across width of objects.
	...
	screen.surface.set_clip(None) #10


def adjust_wall_transparency():
	global wall_transprency_frame

	if (player_y == room_height - 2  #1
		and room_map[room_height - 1][player_x] == 1 #2
		and wall_transprency_frame < 4): #3
		wall_transprency_frame += 1 #Fade wall out.  #4

	if ((player_y < room_height - 2 #5
		or room_map[room_height-1][player_x] != 1)  #6
		and wall_transprency_frame > 0): #7
		wall_transprency_frame -= 1 #8



def show_text(text_to_show, line_number): #1
	if game_over: 
		return
	text_lines = [15,50] #2
	box = Rect((0, text_lines[line_number]), (800,35)) #3
	screen.draw.filled_rect(box,BLACK) #4
	screen.draw.text(text_to_show, (20, text_lines[line_number]), color = GREEN) #5
...
props = { #1
    ...
    24: [45, 0, 2],
    25: [32, 0, 2], 26: [27, 12, 5], # two sides of same door #2
    ...
    71: [0, LANDER_Y, LANDER_X], 72: [0, 0, 0], 73: [27, 4, 6], #3
...
for key,prop in props.items():
	if key != 71: #71 is skipped because it's different each game. #4
		checksum += (prop[0] * key + prop[1] * (key + 1) + prop[2] * (key + 2))

print(len(props), "props") #5
print("Props checksum: ",checksum)
assert len(props) == 37, "Expected 37 prop items"
assert checksum == 61414, "Error in props data" #6
...
def find_object_start_x(): #1
	checker_x = player_x #2
	while room_map[player_y][checker_x] == 255: #3
		checker_x -=1  #4
	return checker_x #5

def get_item_under_player(): #6
	item_x = find_object_start_x() #7
	item_player_is_on = room_map[player_y][item_x] #8
	return item_player_is_on #9 page-159


def pick_up_object():
	global room_map
	item_player_is_on = get_item_under_player() #1
	if item_player_is_on in items_player_may_carry: #2
		room_map[player_y][player_x] = get_floor_type() #3
		add_object(item_player_is_on) #4
...
def add_object(item): #Adds item to inventory. #6
	global selected_item, item_carrying
	in_my_pockets.append(item) #7
	item_carrying = item #8
	selected_item = len(in_my_pockets) - 1 #9
	display_inventory()
	props[item][0] = 0 #Carried objects go into room 0 (off the map). #10


def display_inventory():
	box = Rect((0, 45), (800, 105)) #1
	screen.draw.filled_rect(box, BLACK)

	if len(in_my_pockets) == 0: #2
		return

	start_display = (selected_item // 16) * 16 #3 
	list_to_show = in_my_pockets[start_display : start_display + 16] #4
	selected_marker = selected_item % 16 #5

	for item_counter in range(len(list_to_show)):  #6
		item_number = list_to_show[item_counter]
		image = objects[item_number][0]
		screen.blit(image, (25 + (46 * item_counter), 90)) #7

	box_left = (selected_marker * 46) - 3
	box = Rect((22 + box_left, 85), (40,40)) #8
...
def drop_object(old_y, old_x): #1
	global room_map, props 
	if room_map[old_y][old_x] in [0,2,39]: #places you can drop things #2
		props[item_carrying][0] = current_room #3
		props[item_carrying][1] = old_y
		props[item_carrying][2] = old_x
		room_map[old_y][old_x] = item_carrying #4
		show_text("You have dropped " + objects[item_carrying][3], 0)
		sounds.drop.play()
		remove_object(item_carrying) #5
		time.sleep(0.5)
	else: #This only happens if there is already a prop here #6
...
def remove_object(item): #Takes item out of inventory
	...
	in_my_pockets.remove(item) #7
	selected_item = selected_item -1 #8
	if selected_item < 0: #9
		selected_item = 0
	if len(in_my_pockets) == 0: # If they're not carrying anything #10
...
def examine_object():
	item_player_is_on = get_item_under_player() #1
	left_tile_of_item = find_object_start_x() #2
	if item_player_is_on in [0,2]: # Don't describe the floor #3
		return

	description = "You see: " + objects[item_player_is_on][2] #4
	for prop_number, details in props.items(): #5
		#props = objects number: [room_number, y, x]
		if details[0] == current_room: # if prop is in the room #6
			...
			if (details[1] == player_y 
				...
				add_object(prop_number) #7
				description = "You found " + objects[prop_number][3] #8
				sounds.combine.play()
	show_text(description, 0) #9
	time.sleep(0.25) #10
...
def use_object():
	...
	use_message = "You fiddle around with it but don't get anywhere." #1
	standard_responses = { #2
	...
	item_player_is_on = get_item_under_player() #3
	for this_item in [item_player_is_on, item_carrying]: #4
		if this_item in standard_responses: #5
			use_message = standard_responses[this_item] #6

	if item_carrying == 70 or item_player_is_on == 70: #1
		use_message = "Banging tunes!"
		sounds.steelmusic.play(2)

	elif item_player_is_on == 11: #2
		use_message = "AIR: " + str(air) + "% / ENERGY " + str(energy) + "% /" #3
		...
		# Retrun to stop another object use accidentally overriding this.
		return #4

	elif item_carrying == 60 or item_player_is_on == 60:
		use_message = "You fix " + objects[60][3] + " to this suit" #5
		air_fixed = True
		air = 90
		air_countdown()
		remove_object(60)
		...
		use_message = "You dig..."
		if (current_room == LANDER_SECTOR and player_offset_x == LANDER_X and player_y == LANDER_Y):
			add_object(71)
	...
	elif item_player_is_on == 42: #1
		if current_room == 27: #2
			open_door(26) #3
		props[25][0] = 0 # Door from RM32 to engineering bay #4
		props[26][0] = 0 # Door inside engineering bay
		clock.schedule_unique(shut_engineering_door, 60) #5
		use_message = "You press the buttom" 
	...
	if suit_stitched and air_fixed: # open airlock access
		if current_room == 31 and props[20][0] == 31:
			...
			sounds.say_airlock_open.paly()
			show_text("The computer tells you the airlock is now open.", 1)

	for recipe in RECIPES: #1
		ingredient1 = recipe[0] #2
		ingredient2 = recipe[1]
		combination = recipe[2]
		if (item_carrying == ingredient1 and item_player_is_on == ingredient2) or (item_carrying == ingredient2 and item_player_is_on == ingredient1): #3
		   	use_message = "You combine " + objects[ingredient1][3] 	+ " and " + objects[ingredient2][3] + " to make " + objects[combination][3] #5
		   	if item_player_is_on in props.keys(): #6
		   		props[item_player_is_on][0] = 0 #7
		   		room_map[player_y][player_x] = get_floor_type() #8
		   	in_my_pockets.remove(item_carrying) #9
		   	add_object(combination) #10
		   	sounds.combine.play()

	# {key object number: door object number}
	ACCESS_DICTIONARY = {79:22, 80:23, 81:24} #6
	if item_carrying in ACCESS_DICTIONARY: #7
		door_number = ACCESS_DICTIONARY[item_carrying]
		if props[door_number][0] == current_room: #8
			sounds.say_doors_open.play() #9
			sounds.doors.play()
			open_door(door_number)



	show_text(use_message, 0) #7
	time.sleep(0.5) 
...
def open_door(opening_door_number): # 1
	global door_frames, door_shadow_frames
	global door_frame_number, door_object_number
	door_frames = [images.door1, images.door2, images.door3, images.door4, images.floor] #2
	# (Final frame restores shadow ready for when door reappears).
	door_shadow_frames = [images.door1_shadow, images.door2_shadow, images.door3_shadow,\
						 images.door4_shadow, images.door_shadow]
	door_frame_number = 0
	door_object_number = opening_door_number
	do_door_animation() #3

def close_door(closing_door_number): #4
	global door_frames, door_shadow_frames
	global door_frame_number, door_object_number, player_y
	door_frames = [images.door4, images.door3, images.door2, images.door1, images.door] #5
	door_shadow_frames = [images.door4_shadow, images.door3_shadow, images.door2_shadow,\
						 images.door1_shadow, images.door_shadow]
	door_frame_number = 0
	door_object_number = closing_door_number
	# If player is in same row as a door, they must be in open doorway
	if player_y == props[door_object_number][1]: #6
		if player_y == 0: # if in the top doorway #7
			player_y = 1 # move them down #8
		else:
			player_y == room_height - 2 # move them up #9
	do_door_animation() #10

def do_door_animation():
	global door_frames, door_frame_number, door_object_number, objects
	objects[door_object_number][0] = door_frames[door_frame_number] #1
	objects[door_object_number][1] = door_shadow_frames[door_frame_number]
	door_frame_number += 1 #2
	if door_frame_number == 5: #3
		if door_frames[-1] == images.floor: #4
			props[door_object_number][0] = 0 # Remove door from props list #5
		# Regenerate room map from the props
		# to put the door in the room if required.
		generate_map() #6
	else: #7
		clock.schedule(do_door_animation,0.15) #8


def shut_engineering_door():
	global current_room, door_room_number, props
	props[25][0] = 32 #1 #Door from room 32 to the engineerin gbay.
	props[26][0] = 27 #2 Door inside  engineering bay.
	generate_map() #3 Add door to room_map for if in affected room.
	if current_room == 27: #4
		close_door(26) #5
	if current_room == 32: #6
		close_door(25) #7
...
def door_in_room_26():
	...
	frames = [images.door, images.door1, images.door2, images.door3, images.door4, images.floor]  #1
	...
	if ((player_y ==8 and player_x == 2) or props[63] == [26,8,2]) and props[21][0] == 26:
 		airlock_door_frame += 1 #4
 		if airlock_door_frame == 5: #5
 			props[21][0] = 0 # Remove door from map when fully open.
 			room_map[0][1] = 0
 			room_map[0][2] = 0
 			room_map[0][3] = 0

	if ((player_y != 8 or player_x != 2) and props[63] != [26,8,2]) and airlock_door_frame > 0: #6
	...
	objects[21][0] = frames[airlock_door_frame] #7
...
def draw_energy_air():
	...
	screen.draw.filled_rect(box, BLACK) # Clear air bar. #1
	screen.draw.text("AIR", (20, 766), color=BLUE) #2
	screen.draw.text("ENERGY", (180,766), color=YELLOW) #3

	if air > 0: #4
		box = Rect((50,765), (air, 20)) #5
		screen.draw.filled_rect(box, BLUE) # Draw new air bar. #6
...
def end_the_game(reason): #1
	global game_over
	show_text(reason, 1) #2
	game_over = True #3
	sounds.say_mission_fail.play()
	sounds.gameover.play()
	screen.draw.text("GAME OVER", (120, 400), color = "white", fontsize = 128, shadow = (1, 1), scolor = "black") #4
...
hazard_data = {
	#room number: [[y,x, direction, bounce addition to direction]] #1
...
def hazard_start():
	...
	if current_room in hazard_data.keys(): #1
		current_room_hazards_list = hazard_data[current_room] #2
		for hazard in current_room_hazards_list: #3
			hazard_y = hazard[0]
			hazard_x = hazard[1]
			hazard_map[hazard_y][hazard_x] = 49 + (current_room % 3) #4
...
def hazard_move(): 
	...
	for hazard in current_room_hazards_list:
		...
		if (hazard_y == player_y and hazard_x == player_x) or \
		   (hazard_y == from_player_y and hazard_x == from_player_x and player_frame > 0): #3
	   		sounds.ouch.play()
	   		deplete_energy(5)
	   		hazard_should_bounce = True

	   	#Stop hazard going out of the doors #4
		...
		if hazard_y == -1:
	   	...
	   	#Stop when hazard hits scenery or another hazard. #5
		if room_map[hazard_y][hazard_x] not in items_player_may_stand_on \
	   		or hazard_map[hazard_y][hazard_x] != 0:
	   		hazard_should_bounce = True

		if hazard_should_bounce: #6
	   		hazard_y = old_hazard_y #Move back to last valid position.
	   		hazard_x = old_hazard_x
	   		hazard_direction += hazard[3] #7
	   		if hazard_direction > 4: #8
	   			hazard_direction -= 4
	   		if hazard_direction < 1:
	   			hazard_direction += 4
	   		hazard[2] = hazard_direction #9
		hazard_map[hazard_y][hazard_x] = 49 + (current_room % 3) #10
		hazard[0] = hazard_y
		hazard[1] = hazard_x
...
generate_map()
clock.schedule_interval(adjust_wall_transparency,0.05) #9
clock.schedule_unique(display_inventory,1) #10
clock.schedule_unique(draw_energy_air, 0.5)
clock.schedule_unique(alarm, 10)
# A higher number below gives a longer time limit.
clock.schedule_interval(air_countdown, 12)
